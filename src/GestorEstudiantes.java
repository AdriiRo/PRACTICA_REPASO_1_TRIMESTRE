...
public class GestorEstudiantes {
    ...
    public void anadirEstudiante(Estudiante[] estudiantes) throws InterruptedException {
        ...
        if (contadorEstudiantes < estudiantes.length) {
            ...
            String autoGeneratedEmail = inputApellidos.toLowerCase() + "." + inputNombre.toLowerCase()
                    + (ANO_EN_CURSO - inputEdad) + "@tiernoparla.es"; // Variable que almacena un email generado a
                                                                      // partir del nombre y de la edad
    ...
    public void buscarPorEmail(Estudiante[] estudiantes) {
        ...
        for (int i = 0; i < estudiantes.length; i++) {
            if (inputEmail.equals(estudiantes[i].getEmail())) {
                System.out.println(
                        "Alumno encontrado: " + estudiantes[i].getNombre() + " " + estudiantes[i].getApellido());
            } else {
